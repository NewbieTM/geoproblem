...
from sentinelhub import (
    SentinelHubCatalog, SentinelHubRequest, BBox, CRS, MimeType, DataCollection,
...
def download_new_bands(bbox, time_interval, config, target_size, output_dir):
    ...
    with rasterio.open(
            ...
            width=target_size[0],
            count=2,  # Мы записываем два канала
        ...
        # Запись двух каналов (первый канал для B04, второй для B08)
        dst.write(response[0][:, :, 0], indexes=1)  # Запись первого канала (B04)
        dst.write(response[0][:, :, 1], indexes=2)  # Запись второго канала (B08)
...
if __name__ == "__main__":
    last_path_in = r"D:\omela\output_data_sentinel\S2A_MSIL1C_20180611T065621_N0206_R063_T42VUP_20180611T090058.SAFE"
    output_dir = r"D:\omela\new_band_data\2A_MSIL1C_20180611T065621_N0206_R063_T42VUP_20180611T090058.SAFE"
    config = make_config("6cec2602-3a03-4980-b48a-4d17f8f59bbe")
    start_date = datetime(2023, 5, 2)
    end_date = datetime(2023, 9, 30)

    for el in os.listdir(last_path_in):
        file_path = os.path.join(last_path_in, el, 'response.tiff')
        try:
            with rasterio.open(file_path) as src:
                ...
                while current_date <= end_date:
                    ...
                    try:
                        ...
                        if compare_masks(old_clm_mask, new_mask, threshold=0.95):
                            print(f"Masks matched for {current_date.date()}, downloading new bands...")
                            download_new_bands(target_bbox, time_interval, config, needed_size, os.path.join(output_dir, el))
                            break
                        else:
                            print(f"Masks do not match for {current_date.date()}.")
                    except Exception as e:
                        print(f"Error downloading or processing mask for {current_date.date()}: {e}")
